// =============================================================
// Auto-generated CNN_top (readable, flexible)
// =============================================================
module CNN_top #(
    ...
    // Conv1: Conv2D
    // ------------------------------------------------------------
    wire [1*DATA_WIDTH-1:0] Conv1_conv_out;
    wire Conv1_conv_valid;
    ...
    Conv2D #(
        ...
        .data_in(data_in),
        .conv_out(Conv1_conv_out),
        .conv_valid(Conv1_conv_valid)
    );

    // ------------------------------------------------------------
    // Maxpool1: Max2D
    // ------------------------------------------------------------
    wire [1*DATA_WIDTH-1:0] Maxpool1_maxp_out;
    wire Maxpool1_maxp_valid;
    ...
    Max2D #(
        ...
        .data_in(Conv1_out),
        .maxp_out(Maxpool1_maxp_out),
        .maxp_valid(Maxpool1_maxp_valid)
    );
    ...
    Layer_1 #(
        ...
        .x_valid(Maxpool1_maxp_valid),
        .x_in(Maxpool1_maxp_out),
        .o_valid(o1_valid),
        .x_out(x1_out)
    );

    // ---------------- State machine for Layer 1 ----------------
    reg state_1;
    integer count_1;
    ...
    // ------------------------------------------------------------
    // Final-layer packed hold + readout + maxFinder
    // ------------------------------------------------------------
   // reg [10*DATA_WIDTH-1:0] holdData_final;
   // assign axi_rd_data = holdData_final[DATA_WIDTH-1:0];

   // always @(posedge clock) begin
   //     if (o3_valid[0] == 1'b1)
   //         holdData_final <= x3_out;
   //     else if (axi_rd_en) begin
   //         holdData_final <= holdData_final >> DATA_WIDTH;
   //     end
   // end
